...
/* Process management */
void create_processes(int lines, int nProc, const char *filename, const char *pattern);
...
void init_process_table(int *nProc,int lines);
...
void parse_argv(int argc, char *argv[], char **filename, char **pattern, int *lines, int *nProc);
...
void create_processes(int lines, int nProc, const char *filename, const char *pattern) {
  FILE *fp;
  char line[PATH_MAX], line_number_str[PATH_MAX];
  int line_number = 0;
  int n_lines_proc = lines / nProc;
  ...
  while (((lines / nProc) != 0) && fgets(line, sizeof(line)*n_lines_proc, fp) != NULL)) { 
    ...
    line_number++;
    lines = lines - n_lines_proc;
  }
  while(fgets(line, sizeof(line)*lines, fp)!= NULL){ 
    sprintf(line_number_str, "%d",line_number);
    create_processes_by_class(PATTERN,1,line_number * 2, line, line_number_str, pattern);
    create_processes_by_class(COUNTER,1,line_number * 2 + 1,line, line_number_str, NULL);
  }
...
void init_process_table(int *nProc,int lines) {
  ...
  if((lines / *nProc) < 1){
    printf("\n[MANAGER %d] less nº of file lines than processes, generating two processes.\n",getpid());
    g_nProcesses = 2;
    *nProc = g_nProcesses;
  }
  else{
    g_nProcesses = *nProc * 2;
  }
...
void parse_argv(int argc, char *argv[], char **filename, char **pattern, int *lines, int *nProc) {
  ...
  if (argc != 4) {
    fprintf(stderr, "Error. Use: ./exec/manager <file> <pattern> <nºProc>.\n");    
    exit(EXIT_FAILURE); 
  }
